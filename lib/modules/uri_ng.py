...
def main():
    ...
    # return_headers = module.params['return_headers']
    ...
    if 'content_type' in uresp:
        ...
        u_content = to_text(content, encoding=content_encoding)
        u_resp = to_text(uresp, encoding=content_encoding)
        u_info = to_text(resp, encoding=content_encoding)
    if resp['status'] not in status_code:
        uresp['msg'] = 'Status code was %s and not %s: %s' % (resp['status'], status_code, uresp.get('msg', ''))
        module.fail_json(content=u_content, resp=u_resp, info=u_info, **uresp)
    elif return_content:
        module.exit_json(content=u_content, resp=u_resp, info=u_info, **uresp)
    else:
